...
MAX_TOOL_ROUNDS = 16
RAW_TOOL_ONLY_PROMPT = "SkillExecutor is disabled in this app. Do not call SkillExecutor. Use raw tools directly."
...
def run_agent(user_question, *flat_args):
    ...
    for round_id in range(1, MAX_TOOL_ROUNDS + 1):
        try:
            model_output = model.generate(conversation, extra_system_prompt=RAW_TOOL_ONLY_PROMPT)
        except Exception as exc:
            chat_msgs.append(
                {
                    "role": "assistant",
        ...
        if api_name == "SkillExecutor":
            chat_msgs.append(
                {
                    "role": "assistant",
                    "content": f"[Round {round_id}] ❌ Observation\nSkillExecutor 已禁用，请直接选择底层 raw tools 继续执行。",
                }
            )
            yield emit(None)
            conversation.append({"role": "assistant", "content": model_output})
            conversation.append(
                {
                    "role": "user",
                    "content": "SkillExecutor is disabled. Choose raw tools directly from the tool list and continue.",
                }
            )
            continue
