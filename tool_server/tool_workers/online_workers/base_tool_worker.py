...
class BaseToolWorker:
    ...
    def register_to_controller(self):
        ...
        logger.info(
            f"Register to controller: url={url}, worker_addr={self.worker_addr}, "
            f"model_name={self.model_name}, host={self.host}, port={self.port}, payload={data}"
        )
        ...
        try:
            ...
            logger.error(
                f"Register to controller request failed: url={url}, worker_addr={self.worker_addr}, "
                f"model_name={self.model_name}, host={self.host}, port={self.port}, error={repr(e)}"
            )
            raise RuntimeError(
                f"Failed to register to controller {url}: request error: {repr(e)}"
            ) from e

        if r.status_code != 200:
            logger.error(
                f"Register to controller failed: url={url}, status_code={r.status_code}, "
                f"response_text={r.text!r}, worker_addr={self.worker_addr}, "
                f"model_name={self.model_name}, host={self.host}, port={self.port}, payload={data}"
            )
            raise RuntimeError(
                f"Failed to register to controller {url}: "
                f"status_code={r.status_code}, response_text={r.text!r}"
            )

        logger.info(
            f"Register to controller succeeded: url={url}, status_code={r.status_code}, "
            f"response_text={r.text!r}"
        )
