...
## 1. 修改启动配置文件
在 `all_service_example_local.yaml` 列表的末尾，添加新工具的启动配置，例如：
...
## 3. 单个工具效果测试
采用代码 `/home/ubuntu/01_Code/OpenEarthAgent/scripts/tools_test/tools_single_test.py` 测试工具效果（注意参考tools的 input和output修改输入输出）

> 测试成功之后，将对应的模块添加到 `scripts/tools_test/tools_test.py` 中，补全all工具的测试代码

## 4. 补全Agent提示词
在 `/home/ubuntu/01_Code/OpenEarthAgent/tool_server/tf_eval/utils/rs_agent_prompt.py` 中添加模块描述。

## 5. 补全界面输入
在 `app/app_new.py` 文件中添加相关的示例进行测试（保存图片为png格式，tiff格式有点问题）, 涉及到参数输入输出，需要在 `app_new.py` 中添加 `TOOL_ARG_BINDINGS`、`GPKG_REQUIRED_TOOLS` 等工具的输入参数映射
